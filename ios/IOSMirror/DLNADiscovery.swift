...
/// Covers Samsung AllShare / Screen Mirror TVs and any UPnP AV renderer.
final class DLNADiscovery {
    ...
    private func performSearch() {
        ...
        while running {
            ...
            handleResponse(response, seen: &seen)
        }
    }

    // MARK: - Response parsing

    private func handleResponse(_ response: String, seen: inout Set<String>) {
        var location: String?
        var usn: String?
        ...
        guard let loc = location, let id = usn,
              !seen.contains(id), knownDevices[id] == nil
        else { return }
        seen.insert(id)
        guard let url = URL(string: loc) else { return }
        fetchDescription(from: url, id: id)
    }
